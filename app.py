...
def main():
    ...
    tab_names = [
        ...
        # Custom dashboard tab where user can assemble widgets
        'Custom Dashboard'
    ]
    ...
    with tab7:
        st.subheader("Custom Dashboard — Build your layout")

        # Persist layouts per user (uses session name or fallback)
        user_key = (st.session_state.get("fb_user_name") or "default_user").replace(" ", "_")
        ...
        layout_path = os.path.join(layouts_dir, f"layout_{user_key}.json")

        # Widget -> dataframe column mapping for metric selection in modals
        METRIC_DISPLAY_MAP = {
            "Clicks": "clicks",
            "Impressions": "impressions",
            "Landing Page Views": "landing_page_view",
            "Conversations": "messaging_conversation_starts",
            "Spend": "spend",
            "Reach": "reach",
        }

        # Available widgets that can be added to the custom dashboard
        AVAILABLE_WIDGETS = [
            "Key Metrics",
            "Daily Trends",
            "Campaign Spend",
        ...
        def _normalize_layout(raw):
            """Ensure layout is list of dicts: {widget: str, settings: dict}"""
            out = []
            if not raw:
                return []
            for item in raw:
                if isinstance(item, str):
                    out.append({"widget": item, "settings": {}})
                elif isinstance(item, dict):
                    w = item.get("widget") or item.get("name") or item.get("title")
                    s = item.get("settings", {})
                    if w:
                        out.append({"widget": w, "settings": s})
            return out

        if "custom_layout" not in st.session_state:
            # try load from disk and normalize
            try:
                if os.path.exists(layout_path):
                    with open(layout_path, "r", encoding="utf-8") as f:
                        raw = json.load(f)
                    st.session_state.custom_layout = _normalize_layout(raw)
                else:
                    st.session_state.custom_layout = []
            except Exception:
                st.session_state.custom_layout = []

        # Defensive normalization: ensure layout is list of dicts even if session previously stored strings
        st.session_state.custom_layout = _normalize_layout(st.session_state.custom_layout)

        col_add, col_save = st.columns([3, 1])
        with col_add:
            add_choice = st.selectbox("Add widget", options=AVAILABLE_WIDGETS, key=f"add_choice_{user_key}")
            if st.button("➕ Add widget", key=f"add_btn_{user_key}"):
                # add as dict with empty/default settings
                default_settings = {}
                if add_choice == "Top Ads":
                    default_settings = {"top_n": 5, "metric": "Clicks"}
                elif add_choice == "Daily Trends":
                    default_settings = {"metric": "Clicks"}
                elif add_choice == "Campaign Spend":
                    default_settings = {"metric": "Spend"}
                st.session_state.custom_layout.append({"widget": add_choice, "settings": default_settings})

        with col_save:
            if st.button("💾 Save Layout", key=f"save_layout_{user_key}"):
                try:
                    with open(layout_path, "w", encoding="utf-8") as f:
                        json.dump(st.session_state.custom_layout, f, ensure_ascii=False, indent=2)
                    st.success(f"Layout saved to {layout_path}")
                except Exception as e:
                    st.error(f"Failed to save layout: {e}")

        # Controls to load or reset
        col_l1, col_l2 = st.columns([1,1])
        with col_l1:
            if st.button("🔁 Load Saved Layout", key=f"load_layout_{user_key}"):
                try:
                    if os.path.exists(layout_path):
                        with open(layout_path, "r", encoding="utf-8") as f:
                            raw = json.load(f)
                        ...
                        st.warning("No saved layout found")
                except Exception as e:
                    st.error(f"Failed to load layout: {e}")
        with col_l2:
            if st.button("🗑️ Clear Layout", key=f"clear_layout_{user_key}"):
                st.session_state.custom_layout = []

        st.markdown("---")

        # Display and reorder the current layout (now list of dicts)
        if st.session_state.custom_layout:
            st.markdown("**Your layout (top → bottom):**")
            # copy for safe mutation
            new_layout = list(st.session_state.custom_layout)
            for idx, item in enumerate(list(st.session_state.custom_layout)):
                # Defensive extraction: support legacy string items
                if isinstance(item, str):
                    widget_name = item
                    settings = {}
                elif isinstance(item, dict):
                    widget_name = item.get("widget") or item.get("name") or str(item)
                    settings = item.get("settings", {})
                else:
                    widget_name = str(item)
                    settings = {}
                cols = st.columns([5,1,1,1,1])
                cols[0].markdown(f"**{idx+1}. {widget_name}**")
                if cols[1].button("⬆️", key=f"up_{user_key}_{idx}") and idx > 0:
                    new_layout[idx], new_layout[idx-1] = new_layout[idx-1], new_layout[idx]
                if cols[2].button("⬇️", key=f"down_{user_key}_{idx}") and idx < len(new_layout)-1:
                    new_layout[idx], new_layout[idx+1] = new_layout[idx+1], new_layout[idx]
                if cols[3].button("✖", key=f"del_{user_key}_{idx}"):
                    new_layout.pop(idx)
                # Configure: set a session flag to open an inline expander (works with older Streamlit)
                if cols[4].button("⚙️ Configure", key=f"config_{user_key}_{idx}"):
                    st.session_state[f"cfg_open_{user_key}_{idx}"] = True

            st.session_state.custom_layout = new_layout

            # Render configuration expanders for any opened widgets (inline, not modal)
            for idx, item in enumerate(st.session_state.custom_layout):
                cfg_key = f"cfg_open_{user_key}_{idx}"
                if st.session_state.get(cfg_key, False):
                    widget_name = item.get("widget") if isinstance(item, dict) else (item if isinstance(item, str) else str(item))
                    settings = item.get("settings", {}) if isinstance(item, dict) else {}
                    with st.expander(f"Configure: {widget_name}", expanded=True):
                        st.markdown(f"### Configure: {widget_name}")
                        metric_choice = None
                        top_n = settings.get("top_n", 5)
                        if widget_name in ("Top Ads", "Daily Trends", "Campaign Spend"):
                            metric_display_options = list(METRIC_DISPLAY_MAP.keys())
                            current_metric = settings.get("metric", "Clicks")
                            metric_choice = st.selectbox("Metric", options=metric_display_options, index=max(0, metric_display_options.index(current_metric)) if current_metric in metric_display_options else 0, key=f"cfg_metric_{user_key}_{idx}")
                        if widget_name == "Top Ads":
                            top_n = st.number_input("Top N", min_value=1, max_value=50, value=int(settings.get("top_n", 5)), key=f"cfg_topn_{user_key}_{idx}")

                        col_save_close = st.columns([1,1])
                        if col_save_close[0].button("Save", key=f"cfg_save_{user_key}_{idx}"):
                            new_settings = dict(settings)
                            if metric_choice is not None:
                                new_settings["metric"] = metric_choice
                            if widget_name == "Top Ads":
                                new_settings["top_n"] = int(top_n)
                            st.session_state.custom_layout[idx]["settings"] = new_settings
                            st.session_state[cfg_key] = False
                            st.success("Saved settings")
                        if col_save_close[1].button("Close", key=f"cfg_close_{user_key}_{idx}"):
                            st.session_state[cfg_key] = False

            st.markdown("---")
            st.subheader("Preview")
            # Render preview of each widget using existing dataframes and settings
            for item in st.session_state.custom_layout:
                # Defensive extraction for preview loop
                if isinstance(item, str):
                    widget_name = item
                    settings = {}
                elif isinstance(item, dict):
                ...
                try:
                    if widget_name == "Key Metrics":
                        c1, c2, c3, c4 = st.columns(4)
                        c1.metric("Spend", f"RM {total_spend:,.2f}")
                        c2.metric("Clicks", f"{int(total_clicks):,}")
                        c3.metric("Landing Page Views", f"{int(total_landing_page_views):,}")
                        c4.metric("Reach", f"{int(total_reach):,}")
                    elif widget_name == "Daily Trends":
                        metric_col = METRIC_DISPLAY_MAP.get(settings.get("metric", "Clicks"), "clicks")
                        if not daily_df.empty and metric_col in daily_df.columns:
                            fig_daily = go.Figure()
                            fig_daily.add_trace(go.Scatter(x=daily_df['date_start'], y=daily_df[metric_col], mode='lines', name=metric_col, line=dict(color=PALETTE['series']['clicks'])))
                            style_fig(fig_daily, height=300)
                            st.plotly_chart(fig_daily, use_container_width=True)
                    elif widget_name == "Campaign Spend":
                        if not campaigns_df.empty:
                            fig_spend = px.bar(campaigns_df, x='campaign_name', y=settings.get("metric", "spend") if settings.get("metric") and METRIC_DISPLAY_MAP.get(settings.get("metric")) in campaigns_df.columns else 'spend')
                            fig_spend.update_traces(marker_color=PALETTE['series']['spend'])
                            style_fig(fig_spend, height=300)
                            st.plotly_chart(fig_spend, use_container_width=True)
                    elif widget_name == "Top Ads":
                        n = int(settings.get("top_n", 5))
                        metric_col = METRIC_DISPLAY_MAP.get(settings.get("metric", "Clicks"), "clicks")
                        if not ads_df.empty and metric_col in ads_df.columns:
                            top_clicks = ads_df.nlargest(n, metric_col)[['ad_name', metric_col]]
                            fig_top = px.bar(top_clicks, x=metric_col, y='ad_name', orientation='h')
                            style_fig(fig_top, height=300)
                            st.plotly_chart(fig_top, use_container_width=True)
                    elif widget_name == "Platform Performance":
                        if not platform_df.empty:
                            fig_platform = px.pie(platform_df, values='clicks', names='publisher_platform', hole=0.6)
                            style_fig(fig_platform, height=300)
                            st.plotly_chart(fig_platform, use_container_width=True)
                    elif widget_name == "Age Distribution":
                        if not age_df.empty and 'age' in age_df.columns:
                            fig_age = px.bar(age_df, x='age', y='clicks')
                            style_fig(fig_age, height=300)
                            st.plotly_chart(fig_age, use_container_width=True)
                    elif widget_name == "Ads Table":
                        if not ads_df.empty:
                            st.dataframe(ads_df.head(10), use_container_width=True)
                except Exception as e:
                    st.warning(f"Could not render widget '{widget_name}': {e}")
        else:
            st.info("Add widgets to your layout on the left and click 'Save Layout' to persist it.")
